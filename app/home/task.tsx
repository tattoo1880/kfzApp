...
export default function Task() {
    ...
    const fetchData = async () => {
        try {
            const [userResponse, taskResponse] = await Promise.all([
                ...
                axios.post(`${API_URL}/good/gettodaytaskinfo`, { uid }, {
                    headers: { "Authorization": `Bearer ${token}` },
                })
            ]);
    ...
    const fetchCartData = async (userData?: UserData) => {
        try {
            const shopName = userData?.shop?.shopName ||
                (data && "shop" in data && (data as any).shop?.shopName) ||
                "未知用户";
    ...
    const handleSelectAndDelete = (item: any, index: number) => {
        Alert.alert(
            "确认删除",
            `确定要删除分组 "${item.name || `分组 ${index + 1}`}" 吗？`,
            [
                {
                    text: "取消",
                    style: "cancel"
    ...
    const deleteItem = async (item: any, index: number) => {
        console.log(item)
        try {
            const response = await axios.post(`${API_URL}/sdk/deleteoldgoodsbycid`, {
                cid: item.cid,
                session: uid,
    ...
    const handleClearTasks = async () => {
        try {
            await axios.post(`${API_URL}/good/deltealltask`, {
                uid: uid,
                usernick: (data && "shop" in data && (data as any).shop?.shopName) || "未知用户",
            }, {
                headers: { "Authorization": `Bearer ${token}` }
    ...
    const doTask = async () => {
        ...
        const jwt = useTokenStore.getState().getToken();
        try {
            const res = await axios.post(`${API_URL}/task/send`, {
                uid: uid,
                usernick: shopName,
            }, {
                headers: {
    ...
    return (
        <View style={ styles.container }>
            ...
            <Button
                ...
                onPress={ openModal }
                style={ styles.button }
                buttonColor={ MD3Colors.tertiary40 }
            >
